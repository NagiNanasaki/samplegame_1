...
; キーボード操作を再有効化
[start_keyconfig]
; サンプルテキストのタイマーと DOM 要素を掃除
[iscript]
if (window._sampleTimer) { clearInterval(window._sampleTimer); window._sampleTimer = null; }
$('#cfg_sample_text').remove();
[endscript]
; カレントレイヤーを message0 に戻す（念のためリセット）
...
[iscript]
// 前回のタイマーを停止して要素を削除
if (window._sampleTimer) { clearTimeout(window._sampleTimer); window._sampleTimer = null; }
$('#cfg_sample_text').remove();
...
// IIFE: text/start をクロージャに閉じ込めて autoSpeed でループ
(function() {
  var text = tf.text_sample;
